...
namespace ATools
{
    ...
    public class SnowflakeIdCreator
    {
        ...
        private static int _workerIdBits = 3;                                   //机器码字节数。4个字节用来保存机器码(定义为Long类型会出现，最大偏移64位，所以左移64位没有意义)
        private static long _maxWorkerId = -1L ^ -1L << _workerIdBits;          //最大机器ID
        private static int _sequenceBits = 15;                                  //计数器字节数，10个字节用来保存计数码
